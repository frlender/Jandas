...
class Series {
    ...
    b(expr, __ctx__) {
        ...
        return this.values.map(x => eval(newExpr));
    }
    query(expr, ctx) {
    ...
    sort_values(ascending = true) {
        const idx = (0, df_lib_1._sortIndices)(this.values, false, ascending);
    ...
    op(opStr, ss) {
        if (_.isUndefined(ss)) {
            ...
            if (_.isString(opStr))
                ...
                vals = this.values.map(x => opStr(x));
            ...
            if (ss instanceof Series) {
                util_1.check.op.index(this.index, ss.index);
                util_1.check.op.indexSame(this.index, ss.index);
                ss = ss.values;
            }
    ...
    change(op_str, periods = 1) {
        const df = new DataFrame_1.default(this.values.map(x => [x]), {
    ...
    rolling(window, { min_periods = undefined, center = false, closed = 'right', step = 1 } = {}) {
        const df = new DataFrame_1.default(this.values.map(x => [x]), {
            index: this.index.cp(), columns: [this.name]
        });
        const roll = new df_lib_2.SeriesRolling(df, window, min_periods, center, closed, step);
        return roll;
    }
    isna() {
