...
var DataFrame = /** @class */ (function () {
    ...
    DataFrame.prototype.drop_duplicates = function (labels, options) {
        if (_.isUndefined(options))
            options = {};
        var _a = _.defaults(options, { keep: 'first', axis: 1 }), keep = _a.keep, axis = _a.axis;
        if (axis === 1) {
            var sub = this.loc(null, labels);
            var idx = (0, cmm_1.duplicated)(sub.values, keep);
            return this.loc(idx.map(function (x) { return !x; }));
        }
        else {
            ...
            if (sub instanceof DataFrame)
                sub = sub.transpose();
            var idx = (0, cmm_1.duplicated)(sub.values, keep);
            return this.loc(null, idx.map(function (x) { return !x; }));
        }
    };
