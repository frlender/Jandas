...
var isNum = function (x) { return _.isNumber(x); };
exports.isNum = isNum;
var isStr = function (x) { return _.isString(x); };
exports.isStr = isStr;
...
var check = {
    ...
    mulDataFrame: {
        init: {
            override: function (label, index1, index2) {
                if (index1.shape !== index2.shape)
                    throw ("Index Overriding: The index length of the ".concat(label, " dataframe is not the same as the ").concat(label, " length of the values dataframe."));
            },
            align: function (label, indexIdx, valuesIdx) {
                if (!valuesIdx.is_unique())
                    throw ("Index Align: The ".concat(label, " of the values dataframe is not unique."));
                var diff = _.difference(valuesIdx.values, indexIdx.values);
                if (diff.length > 0)
                    throw ("Index Align: There are values in the index of the ".concat(label, " dataframe that are not in the ").concat(label, " of the values dataframe."));
            }
        }
    }
};
...
function _trans_rg(x, len) {
    ...
    var splits = x.split(':');
    var start_str, end_str, step_str = '1';
    if (splits.length === 2) {
        start_str = splits[0], end_str = splits[1];
    }
    else {
        start_str = splits[0], end_str = splits[1], step_str = splits[2];
    }
    ...
    if (step > 0) {
        ...
        var _a = [parseFloat(start_str),
            parseFloat(end_str)], start = _a[0], end = _a[1];
...
function range(first, second, third) {
    if (second === undefined && third === undefined)
        ...
        return _trans_rg("".concat(first, ":").concat(second, ":").concat(third), third > 0 ? second : (first + 1));
}
exports.range = range;
